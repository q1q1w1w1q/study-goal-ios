...
class LoginVC: BaseViewController, UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate {
	...
	@IBAction func login(_ sender:UIButton) {
		...
		if (emailTextField.text == nil) {
			...
			dataManager.loginStudent(selectedInstituteObject.id, email: emailTextField.text!, password: passwordTextField.text!, completion: { (success, failureReason) -> Void in
				if (success) {
					currentlyLoggedInStudentInstitute = selectedInstituteObject.id
					currentlyLoggedInStudentEmail = self.emailTextField.text!
					currentlyLoggedInStudentPassword = self.passwordTextField.text!
